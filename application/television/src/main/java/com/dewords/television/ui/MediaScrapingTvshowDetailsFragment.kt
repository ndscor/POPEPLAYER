...
import com.dewords.pope.database.models.*
import com.dewords.pope.viewmodel.MediaMetadataFull
import com.dewords.pope.viewmodel.MediaMetadataModel
...
class MediaScrapingTvshowDetailsFragment : DetailsSupportFragment(), CoroutineScope by MainScope(), OnItemViewClickedListener {
    ...
    private lateinit var detailsDescriptionPresenter: com.dewords.television.ui.TvShowDescriptionPresenter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        detailsDescriptionPresenter = com.dewords.television.ui.TvShowDescriptionPresenter()
        arrayObjectAdapterPosters = ArrayObjectAdapter(com.dewords.television.ui.MediaImageCardPresenter(requireActivity(), MediaImageType.POSTER))

        val extras = requireActivity().intent.extras ?: savedInstanceState ?: return
        showId = extras.getString(com.dewords.television.ui.TV_SHOW_ID) ?: return
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.putString(com.dewords.television.ui.TV_SHOW_ID, showId)
        ...
        when (item) {
            ...
            is MediaMetadataWithImages -> {
                launch {
                    ...
                    media?.let {
                        com.dewords.television.ui.TvUtil.showMediaDetail(requireActivity(), it)
                    }
                }
            }
        }
        ...
        tvshow?.let { tvShow ->
            ...
            tvShow.metadata?.let { metadata ->
                ...
                if (metadata.images.any { it.imageType == MediaImageType.BACKDROP }) {
                    val arrayObjectAdapterBackdrops = ArrayObjectAdapter(com.dewords.television.ui.MediaImageCardPresenter(requireActivity(), MediaImageType.BACKDROP))
            ...
            rowsAdapter.setItems(items, object : DiffCallback<Row>() {
                ...
                override fun areContentsTheSame(oldItem: Row, newItem: Row): Boolean {
                    if (oldItem is DetailsOverviewRow && newItem is DetailsOverviewRow) {
                        return oldItem.item as com.dewords.television.ui.MediaItemDetails == newItem.item as com.dewords.television.ui.MediaItemDetails
                    }
                    return true
                }
            })
    ...
    private fun buildDetails(tvShow: MediaMetadataFull) {
        ...
        val videoPresenter = com.dewords.television.ui.VideoDetailsPresenter(requireActivity(), requireActivity().getScreenWidth())

        val activity = requireActivity()
        detailsOverview = DetailsOverviewRow(tvShow)
        rowPresenter.backgroundColor = ContextCompat.getColor(activity, R.color.blue900)
        rowPresenter.onActionClickedListener = OnActionClickedListener { action ->
            when (action.id.toInt()) {
                ...
                ID_START_OVER -> {
                    com.dewords.television.ui.TvUtil.playMedia(activity, viewModel.provider.getAllMedias(viewModel.updateLiveData.value?.seasons))
                }
            }
        }
        selector.addClassPresenter(DetailsOverviewRow::class.java, rowPresenter)
        selector.addClassPresenter(com.dewords.television.ui.VideoDetailsOverviewRow::class.java, videoPresenter)
