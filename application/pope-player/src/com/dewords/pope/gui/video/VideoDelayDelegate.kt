...
import com.dewords.pope.gui.helpers.OnRepeatListenerKey
import com.dewords.pope.gui.helpers.OnRepeatListenerTouch
import com.dewords.pope.gui.helpers.UiTools
import com.dewords.pope.gui.video.VideoPlayerActivity.Companion.KEY_BLUETOOTH_DELAY
import com.dewords.pope.interfaces.IPlaybackSettingsController
import com.dewords.pope.media.DelayValues
...
class VideoDelayDelegate(private val player: VideoPlayerActivity) : View.OnClickListener, IPlaybackSettingsController {
    ...
    fun delayChanged(delayValues: DelayValues, service: PlaybackService) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            delayFirstButton.iconTint = ColorStateList.valueOf(if (delayValues.start == -1L) ContextCompat.getColor(player, R.color.grey400transparent) else ContextCompat.getColor(player, R.color.blue900))
            delaySecondButton.iconTint = ColorStateList.valueOf(if (delayValues.stop == -1L) ContextCompat.getColor(player, R.color.grey400transparent) else ContextCompat.getColor(player, R.color.blue900))
            ...
            viewToAnime?.let { button ->
                val anim = ValueAnimator.ofObject(ArgbEvaluatorCompat(), ContextCompat.getColor(player, R.color.playerbackground), ContextCompat.getColor(player, R.color.blue900focus), ContextCompat.getColor(player, R.color.playerbackground))
